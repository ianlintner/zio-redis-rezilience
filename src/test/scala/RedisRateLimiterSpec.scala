  ...
  override def spec = suite("RedisRateLimiter")(
    test("execute up to max calls immediately") {
      ZIO.scoped {
        for {
          ...
          tbRL   = RedisTokenBucketRateLimiter(redis, testKey, 10, 1.second)
          rl    <- RedisRateLimiter.make(tbRL)
          now   <- Clock.instant
    ...
    test("is not affected by stream chunk size") {
      ZIO.scoped {
        for {
          redis         <- ZIO.service[Redis]
          tbRL           = RedisTokenBucketRateLimiter(redis, testKey, 10, 1.second)
          rl            <- RedisRateLimiter.make(tbRL, 1)
          now           <- Clock.instant
          times1        <-
    ...
    test("will make effects wait for interrupted effects to pass through the rate limiter") {
      ZIO.scoped {
        for {
          redis <- ZIO.service[Redis]
          tbRL   = RedisTokenBucketRateLimiter(redis, testKey, 1, 1.second)
          rl    <- RedisRateLimiter.make(tbRL)
    ...
    RedisExecutor.layer,
    Redis.layer,
  ) @@ timeout(15.seconds) @@ diagnose(15.seconds)
